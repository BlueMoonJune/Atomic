...
public class CommandAtomic implements CommandManager.CommandRegistry {
	...
	public void register(CommandDispatcher<CommandSource> commandDispatcher) {
		...
		ArgumentBuilderLiteral<CommandSource> builder = ArgumentBuilderLiteral.<CommandSource>literal("atomic");
		...
		builder.then(ArgumentBuilderLiteral.<CommandSource>literal("all").executes((c) -> {
			for (String feature : Atomic.FEATURES.keySet()) {
				boolean v = Atomic.FEATURES.get(feature);
				c.getSource().sendMessage(String.format("%s: %s", feature, v ? "§3Enabled" : "§2Disabled"));
			}
			return 1;
		}).then(ArgumentBuilderRequired.<CommandSource, Boolean>argument("value", ArgumentTypeBool.bool()).requires(CommandSource::hasAdmin).executes((c) -> {
			int changed = 0;
			boolean v = c.getArgument("value", Boolean.class);
			for (String feature : Atomic.FEATURES.keySet()) {
